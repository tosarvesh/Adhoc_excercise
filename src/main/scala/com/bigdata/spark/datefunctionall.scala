...
object datefunctionall {
  def main(args: Array[String]) {
    val spark = SparkSession.builder.master("local[*]").appName("datefunctionall").getOrCreate()
    //    val ssc = new StreamingContext(spark.sparkContext, Seconds(10))
    val sc = spark.sparkContext

    import spark.implicits._
    import spark.sql
    val data = "file:///c:\\bigdata\\datasets\\mysqldata.csv"
    ...
    val res = df.select("dob","joindate").withColumn("dob",to_date($"dob", "yyyy-MM-dd"))
      ...
      .withColumn("today",current_date()) // today date you will get
